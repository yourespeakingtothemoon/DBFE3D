...
bool dbf::CubemapTexture::Create(std::string filename, ...)
{
	...
	std::vector<std::string> filenames = generateCMNames(filename, extension);

	// create textures (returns true/false if successful)
	return Load(filenames);
}

bool dbf::CubemapTexture::Load(const std::vector<std::string>& filenames)
{
...
std::vector<std::string> dbf::CubemapTexture::generateCMNames(const std::string& basename, const std::string& extention)
{
	...
	{
		std::string name = basename + suffixes[i] + extention;
